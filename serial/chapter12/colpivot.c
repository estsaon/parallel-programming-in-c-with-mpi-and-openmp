/*
 *   Column pivot version of gaussian elimination.
 *
 *   This C program solves a dense system of linear equations Ax = b using
 *   gaussian elimination with partial pivoting OF COLUMNS followed by back
...
int main (int argc, char *argv[]) {
   ...
   loc = (int *) malloc ((n+1) * sizeof (int));
   for (i = 0; i < n+1; i++) loc[i] = i;
   ...
   for (i = 0; i < n; i++) {
      ...
      magnitude = 0.0;
      for (j = 0; j < n; j++) {
         if (fabs(a[i][loc[j]]) > magnitude) {
            magnitude = fabs(a[i][loc[j]]);
            picked = j;
         }
      }
      tmp = loc[i];
      ...
      /* Reduce all unmarked cols */

      for (j = i+1; j < n; j++) {
         t = a[j][loc[i]] / a[i][loc[i]];
         for (k = i; k < n+1; k++) {
            a[j][loc[k]] -= (a[i][loc[k]] * t);
         }
      }
   }
